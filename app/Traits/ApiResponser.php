...
trait ApiResponser
{
		...
		foreach (request()->query() as $query => $value) {
			...
			if (isset($attribute, $value)) {
				...
				if ($attribute == 'name') {
					...
					$collection = $collection->whereBetween($attribute, $items);
				}else{
					$collection = $collection->whereIn($attribute, $items);
				}
			}
		}
	...
	protected function sortData(Collection $collection, $transformer)
	{
		...
		if (request()->has('sort_desc')) {
			$attribute = $transformer::originalAttribute(request()->sort_desc);
